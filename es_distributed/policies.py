...
# Use the full path to the compatibility module
tf1 = tf.compat.v1
...
class Policy:
    def __init__(self, *args, **kwargs):
        ...
        self.all_variables = tf1.get_collection(tf1.GraphKeys.VARIABLES, self.scope.name)

        self.trainable_variables = tf1.get_collection(tf1.GraphKeys.TRAINABLE_VARIABLES, self.scope.name)
        ...
        placeholders = [tf1.placeholder(v.value().dtype, v.get_shape().as_list()) for v in self.all_variables]
        self.set_all_vars = U.function(
            inputs=placeholders,
            outputs=[],
            updates=[tf1.group(*[v.assign(p) for v, p in zip(self.all_variables, placeholders)])]
        )
    ...
    def rollout(self, env, *, render=False, timestep_limit=None, save_obs=False, random_stream=None):
        """
        If random_stream is provided, the rollout will take noisy actions.
        Updated for Gymnasium/Hopper-v4 compatibility.
        """
        # Modern Gymnasium uses different ways to store max_episode_steps
        try:
            env_timestep_limit = env.spec.max_episode_steps
        except AttributeError:
            env_timestep_limit = 1000  # Default for Hopper
        ...
        for _ in range(timestep_limit):
            ...
            done = terminated or truncated  # Combine for legacy 'done' logic

            rews.append(rew)
            t += 1

            # Rendering is now often handled during gym.make(render_mode="human")
            # so env.render() is usually called automatically or not needed here
            if render:
                env.render()
            if done:
                break
...
def bins(x, dim, num_bins, name):
    ...
    scores_nab = tf1.reshape(scores, [-1, dim, num_bins])
    return tf1.argmax(scores_nab, 2)  # 0 ... num_bins-1
...
class MujocoPolicy(Policy):
    def _initialize(self, ob_space, ac_space, ac_bins, ac_noise_std, nonlin_type, hidden_dims, connection_type):
        ...
        self.nonlin = {'tanh': tf1.tanh, 'relu': tf1.nn.relu, 'lrelu': U.lrelu, 'elu': tf1.nn.elu}[nonlin_type]

        with tf1.variable_scope(type(self).__name__) as scope:
            # Observation normalization
            ob_mean = tf1.get_variable(
                'ob_mean', ob_space.shape, tf1.float32, tf1.constant_initializer(np.nan), trainable=False)
            ob_std = tf1.get_variable(
                'ob_std', ob_space.shape, tf1.float32, tf1.constant_initializer(np.nan), trainable=False)
            in_mean = tf1.placeholder(tf1.float32, ob_space.shape)
            in_std = tf1.placeholder(tf1.float32, ob_space.shape)
            self._set_ob_mean_std = U.function([in_mean, in_std], [], updates=[
                tf1.assign(ob_mean, in_mean),
                tf1.assign(ob_std, in_std),
            ])

            # Policy network
            o = tf1.placeholder(tf1.float32, [None] + list(ob_space.shape))
            a = self._make_net(tf1.clip_by_value((o - ob_mean) / ob_std, -5.0, 5.0))
    ...
    def _make_net(self, o):
        ...
        if ac_bin_mode == 'uniform':
            ...
            a = 1. / (num_ac_bins - 1.) * tf1.to_float(aidx_na) * ac_range_1a + alow[None, :]
            ...
            a = tf1.gather_nd(
                acvals_ak,
                tf1.concat(2, [
                    tf1.tile(np.arange(adim)[None, :, None], [tf1.shape(aidx_na)[0], 1, 1]),
                    tf1.expand_dims(aidx_na, -1)
                ])  # (n,a,2)
            )  # (n,a)
