...
def make_session(single_threaded):
    ...
    if not single_threaded:
        return tf1.InteractiveSession()
    return tf1.InteractiveSession(config=tf1.ConfigProto(inter_op_parallelism_threads=1, intra_op_parallelism_threads=1))
...
def setup(exp, single_threaded):
    ...
    # env = gym.make(exp['env_id'], render_mode="human")
